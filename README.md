...
| `/commit` | For work that accumulated outside the `/fix` loop: splits the working tree into the right number of atomic commits, one per finding, each with a pathspec and a full justification. |
